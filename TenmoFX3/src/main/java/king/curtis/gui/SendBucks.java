...
public class SendBucks {
	...
	getId = new Label("Enter ID of user you wish to send TEbucks to.");
	getAmount = new Label("Enter the amount of TEbucks you wish to send.");
	...
	submit.setOnAction(e -> {
		...
		if ((amount.getText() == null || Objects.equals(amount.getText(), "")) || (user.getText() == null || Objects.equals(user.getText(), ""))){
			...
			blankFields.display(stage, "Transfer Failed", "Transfer Failed: Blank Fields");
			stage.setScene(new MainLoggedInScene().getMainLoggedInScene(stage, currentAuthenticatedUser, currentAccount));
		}

		double holding = Double.parseDouble(amount.getText());
		int receivingUserId = Integer.parseInt(user.getText());

		if (holding <= 0) {
			AlertBox transactionFailed = new AlertBox();
			transactionFailed.display(stage, "Request Failed", "Request Failed: Amount Too Low");
			stage.setScene(new MainLoggedInScene().getMainLoggedInScene(stage, currentAuthenticatedUser, currentAccount));
		} else if (receivingUserId == currentAuthenticatedUser.getUser().getId()){
			AlertBox transactionFailed = new AlertBox();
			...
			if (holding < currentAccount.getBalance().doubleValue()) {
				...
				Account updatedCurrentAccount = accountServices.processTransactionsSent(currentAccount, holding);
				Account updatedReceivingAccount = accountServices.processTransactionReceived(receivingAccount,holding);
				...
				Transfer transfer = new Transfer(2, 2, currentAccount.getAccountId(),
						receivingAccount.getAccountId(), BigDecimal.valueOf(holding));

				transferService.saveTransfer(transfer, currentAuthenticatedUser);
				AlertBox transactionComplete = new AlertBox();
				transactionComplete.display(stage, "Transfer Complete", "Transaction Successful");
				stage.setScene(new MainLoggedInScene().getMainLoggedInScene(stage, currentAuthenticatedUser, currentAccount));
			}
		}
		if (holding > 0) {
			if (holding > currentAccount.getBalance().doubleValue()) {
				AlertBox transactionFailed = new AlertBox();
				transactionFailed.display(stage, "Transfer Failed", "Transaction Failed: Insufficient Funds");
				stage.setScene(new MainLoggedInScene().getMainLoggedInScene(stage, currentAuthenticatedUser, currentAccount));
			}
		}
	});
	...
	layout.getChildren().addAll(generatedUserTable, instructions, getId, user, getAmount, amount, submit);
